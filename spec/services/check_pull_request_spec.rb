...
describe CheckPullRequest do
  ...
  describe '.directory_path' do
    specify do
      expect(checker.directory_path).to eq "#{Rails.root}/tmp/#{repo}/#{number}"
    end
  end

  describe '.files_url' do
    specify do
      expect(checker.files_url).to eq(
  ...
  describe '.comments_url' do
    context 'with no arguements passed' do
      specify do
        expect(checker.comments_url).to eq(
          "https://api.github.com/repos/#{repo}/pulls/#{number}/"\
          "comments?access_token=#{KeyVault[:access_token]}"
        )
      end
    end
    ...
    context 'with access_token set to false' do
      specify do
        expect(checker.comments_url(access_token: false)).to eq(
          "https://api.github.com/repos/#{repo}/pulls/#{number}/comments"
        )
      end
    end
    ...
    context 'with access_token set to true' do
      specify do
        expect(checker.comments_url(access_token: true)).to eq(
          "https://api.github.com/repos/#{repo}/pulls/#{number}/comments"\
    ...
    context 'with id set to 2' do
      specify do
        expect(checker.comments_url(comment_id: 2)).to eq(
          "https://api.github.com/repos/#{repo}/pulls/#{number}/comments"\
          "/2?access_token=#{KeyVault[:access_token]}"
        )
      end
    end
  end
  ...
  describe '.execute' do
    ...
    before do
      expect(HTTP).to receive(:get)
        .with("https://api.github.com/repos/#{repo}/pulls/#{number}/files")
        .and_return(JSON.dump([file_hash]))
      expect(PullRequestFile).to receive(:new).with(file_hash, checker)
        .and_return nil
    ...
    it 'should have cleaned up the files' do
      checker.execute
      expect(File.directory?("#{Rails.root}/tmp/#{repo}/#{number}")).to be false
    end
  end
